...
//    }
     ...
     * @throws UnsupportedEncodingException 
     */
    public Dataset (InputStream file) throws FileNotFoundException, UnsupportedEncodingException {
        toponyms = new ArrayList<Toponym>();
        groups = new HashMap<String, List<Toponym>>();
        BufferedReader src = new BufferedReader(new InputStreamReader(file, ENCODING));
        String line;
        try {
            while ((line = src.readLine()) != null){
                ...
                if (parts[2].isEmpty() || parts[3].isEmpty()){
                    newElement = new Toponym(parts[0], parts[4]);
                    
                } else {
                    newElement = new Toponym(parts[0].trim(), 
